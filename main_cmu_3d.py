...
def main(opt):
    ...
    if not opt.is_eval:
        ...
        data_loader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=16, pin_memory=True)
        valid_dataset = CMU_Motion3D.CMU_Motion3D(opt, split=2)
        print('>>> Validation dataset length: {:d}'.format(valid_dataset.__len__()))
        valid_loader = DataLoader(valid_dataset, batch_size=opt.test_batch_size, shuffle=True, num_workers=16,
                                  pin_memory=True)
    test_loader = {}
    ...
    for act in acts:
        ...
        test_loader[act] = DataLoader(test_dataset, batch_size=opt.test_batch_size, shuffle=False, num_workers=16,
                                      pin_memory=True)
...
def eval(opt):
    ...
    model_path_len = './{}/ckpt_best.pth.tar'.format(opt.ckpt)
    print(">>> loading ckpt len from '{}'".format(model_path_len))
    ckpt = torch.load(model_path_len, map_location='cuda:0',weights_only=False)
    net_pred.load_state_dict(ckpt['state_dict'])
    ...
    for act in acts:
        ...
        dim_used = dataset.dim_used
        data_loader[act] = DataLoader(dataset, batch_size=opt.test_batch_size, shuffle=False, num_workers=16,
                                      pin_memory=True)
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None, dim_used=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        input_gcn_dct = torch.matmul(dct_m, input_gcn)
        input_gcn_dct = input_gcn_dct.reshape(-1, seq_in + out_n, len(dim_used)//3, 3)
        with torch.amp.autocast('cuda'):
            p3d_out_all_4, decorrelation_loss = net_pred(
                input_gcn_dct)  # torch.Size([32, 20, 22, 3])输入 torch.Size([32, 20, 22, 3])输出

        p3d_out_all_4 = p3d_out_all_4.reshape([-1, seq_in + out_n, len(dim_used)])
