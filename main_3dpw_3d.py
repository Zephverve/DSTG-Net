...
def main(opt):
    ...
    test_loader = DataLoader(test_dataset, batch_size=opt.test_batch_size, shuffle=False, num_workers=16,
                             pin_memory=True)
...
def eval(opt):
    ...
    model_path_len = './{}/ckpt_best.pth.tar'.format(opt.ckpt)
    print(">>> loading ckpt len from '{}'".format(model_path_len))
    ckpt = torch.load(model_path_len, weights_only=False)
    net_pred.load_state_dict(ckpt['state_dict'])
    ...
    dim_used = dataset.dim_used
    data_loader = DataLoader(dataset, batch_size=opt.test_batch_size, shuffle=False, num_workers=16,
                                  pin_memory=True)
